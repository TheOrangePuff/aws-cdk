...
describe('stacks', () => {
  test('creates a custom resource and supporting resources in main stack', () => {
    new cloudfront.experimental.NodejsEdgeFunction(stack, 'MyFn', defaultEdgeFunctionProps());
  ...
  test('only one cross-region stack is created for multiple functions', () => {
    new cloudfront.experimental.NodejsEdgeFunction(stack, 'MyFn1', defaultEdgeFunctionProps());
    new cloudfront.experimental.NodejsEdgeFunction(stack, 'MyFn2', defaultEdgeFunctionProps());
  ...
  test('can set the stack id for each function', () => {
    ...
    new cloudfront.experimental.NodejsEdgeFunction(stack, 'MyFn1', defaultEdgeFunctionProps(fn1StackId));
    const fn2StackId = 'edge-lambda-stack-testregion-2';
    new cloudfront.experimental.NodejsEdgeFunction(stack, 'MyFn2', defaultEdgeFunctionProps(fn2StackId));
  ...
  test('a single EdgeFunction used in multiple stacks creates mutiple stacks in us-east-1', () => {
    ...
    new cloudfront.experimental.NodejsEdgeFunction(firstStack, 'MyFn', defaultEdgeFunctionProps());
    new cloudfront.experimental.NodejsEdgeFunction(secondStack, 'MyFn', defaultEdgeFunctionProps());
...
test('addAlias() creates alias in function stack', () => {
  const fn = new cloudfront.experimental.NodejsEdgeFunction(stack, 'MyFn', defaultEdgeFunctionProps());
...
test('mutliple aliases with the same name can be added to the same stack', () => {
  const fn1 = new cloudfront.experimental.NodejsEdgeFunction(stack, 'MyFn1', defaultEdgeFunctionProps());
  const fn2 = new cloudfront.experimental.NodejsEdgeFunction(stack, 'MyFn2', defaultEdgeFunctionProps());
  fn1.addAlias('live');
  fn2.addAlias('live');
...
test('cross-region stack supports new-style synthesis with assets', () => {
  ...
  new cloudfront.experimental.NodejsEdgeFunction(stack, 'MyFn', {
...
test('SSM parameter name is sanitized to remove disallowed characters', () => {
  new cloudfront.experimental.NodejsEdgeFunction(stack, 'My Bad#Fn$Name-With.Bonus', defaultEdgeFunctionProps());
